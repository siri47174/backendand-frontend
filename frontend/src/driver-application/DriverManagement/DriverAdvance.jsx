...
const DriverAdvance = () => {
  ...
  const fetchAdvanceRequests = async () => {
    try {
      const pendingRes = await axios.get(API_BASE_URL + "/advance/pending");
      setAdvances(pendingRes.data);
      const recordsRes = await axios.get(API_BASE_URL + "/advance/records");
  ...
  useEffect(() => { fetchAdvanceRequests(); }, []);

  const handleApproval = async (advanceId, status, requestedAmount) => {
    let approvedAmount = 0;
    if (status === "Approved") {
      approvedAmount = parseFloat(prompt(`Enter approved amount:`));
      if (isNaN(approvedAmount) || approvedAmount <= 0 || approvedAmount > requestedAmount) {
        alert(`Invalid amount. Must be a number and <= ₹${requestedAmount}`);
        return;
      }
    }
    try {
      await axios.post(API_BASE_URL + "/advance/approve", {
        advanceId, status, approvedAmount,
        approvedAt: new Date(), adminId: "admin123", adminName: "Admin User",
      });
      alert(`Advance ${status} successfully`);
      fetchAdvanceRequests();
    } catch (error) {
  ...
  const handleDownload = () => {
    const csv = `Driver Id,Driver Name,Date,Requested Amount,Approved Amount,Status\n` +
      filteredRecords.map(r =>
        `${r.driverId},${r.driverName},${r.month},₹${r.requestedAmount},${r.approvalStatus === "Approved" ? `₹${r.approvedAmount}` : "-"},${r.approvalStatus}`
      ).join("\n");
  ...
  const filteredRecords = records.filter(record => {
    const recordDate = new Date(record.month);
    const fromValid = startDate ? new Date(startDate) <= recordDate : true;
    const toValid = endDate ? new Date(endDate) >= recordDate : true;
    const statusValid = statusFilter === "All" || record.approvalStatus === statusFilter;
    return (
      (record.driverId.toLowerCase().includes(searchTerm.toLowerCase()) ||
        record.driverName.toLowerCase().includes(searchTerm.toLowerCase())) &&
      fromValid && toValid && statusValid
    );
  });
  ...
  return (
    <div style={styles.container}>
      <div style={styles.pageHeader}>DRIVER ADVANCE</div>

      <div style={styles.innerPad}>

        {/* Add Manual Request */}
        <div style={{ marginBottom: "16px" }}>
          <button style={styles.btnBlack} onClick={() => setShowManualForm(!showManualForm)}>
            + Add Manual Advance Request
          </button>
          {showManualForm && (
            <ManualRequestForm
        ...
        {/* Pending Requests */}
        <div style={styles.sectionTitle}>Pending Advance Requests</div>
        {loading ? (
          <p style={{ color: "#888", fontSize: "14px" }}>Loading advance requests...</p>
        ) : advances.length === 0 ? (
          <p style={{ color: "#aaa", fontSize: "14px" }}>No pending advance requests.</p>
        ) : (
          <div style={{ overflowX: "auto" }}>
            <table style={styles.table}>
              <thead>
                <tr>
                  {["Driver ID", "Driver Name", "Month", "Requested Amount", "Actions"].map(h => (
              ...
              <tbody>
                {advances.map((advance, i) => (
                  <tr key={advance._id}
                    ...
                    <td style={styles.td}>{advance.driverId}</td>
                    <td style={styles.td}>{advance.driverName}</td>
                    <td style={styles.td}>{advance.month}</td>
                    <td style={styles.td}><b>₹{advance.requestedAmount}</b></td>
                    <td style={styles.td}>
                      <button style={styles.btnApprove}
                        onClick={() => handleApproval(advance._id, "Approved", advance.requestedAmount)}>
                        Approve
                      </button>
                      <button style={styles.btnReject}
                        onClick={() => handleApproval(advance._id, "Rejected", advance.requestedAmount)}>
                        Reject
                      </button>
                    </td>
                  </tr>
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={styles.table}>
            <thead>
              <tr>
                {["Driver ID", "Driver Name", "Date", "Requested Amount",
                  "Approved Amount", "Status"].map(h => (
                  <th style={styles.th} key={h}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filteredRecords.length > 0 ? filteredRecords.map((record, i) => (
                <tr key={record._id}
                  ...
                  <td style={styles.td}>₹{record.requestedAmount}</td>
                  <td style={styles.td}>
                    {record.approvalStatus === "Approved" ? `₹${record.approvedAmount}` : "—"}
                  </td>
                  <td style={styles.td}>{getStatusBadge(record.approvalStatus)}</td>
                </tr>
              )) : (
                <tr>
                  <td colSpan={6}
