...
// const badge = s=>s==="Paid"?"badge-green":s==="On Hold"?"badge-red":"badge-yellow";
// const empty = { vendorName:"", vehicleNumber:"", invoiceNumber:"", tripNumber:"", grossAmount:"", deductions:"", netAmount:"", paymentDate:new Date().toISOString().split("T")[0], paymentMode:"Bank Transfer", utrNumber:"", remarks:"", status:"Pending" };
// export default function Payment() {
//   const [form, setForm] = useState(empty);
//   const [records, setRecords] = useState([]);
//   const [search, setSearch] = useState("");
//   const load = ()=>axios.get(`${BASE}/advance/vendor-payment`).then(r=>setRecords(r.data||[])).catch(()=>{});
//   useEffect(()=>{load();},[]);
//   const ch = e=>{const f={...form,[e.target.name]:e.target.value};if(f.grossAmount&&f.deductions)f.netAmount=((+f.grossAmount)-(+f.deductions)).toFixed(2);setForm(f);};
//   const save = async()=>{try{await axios.post(`${BASE}/advance/vendor-payment`,form);setForm(empty);load();}catch{alert("Save failed");}};
//   const filtered = records.filter(r=>(r.vendorName||"").toLowerCase().includes(search.toLowerCase()));
//   return (
//     <div style={S.container}>
//       <h2 className="sendo-heading">Vendor Payments</h2>
//       <div style={S.formGrid}>
//         {[["Vendor Name","vendorName"],["Vehicle Number","vehicleNumber"],["Invoice Number","invoiceNumber"],["Trip Number","tripNumber"],["Gross Amount (₹)","grossAmount"],["Deductions (₹)","deductions"],["Net Amount (₹)","netAmount"],["Payment Date","paymentDate"],["UTR / Reference","utrNumber"],["Remarks","remarks"]].map(([l,n])=>(
//           <div style={S.formGroup} key={n}><label style={S.label}>{l}</label><input style={S.input} readOnly={n==="netAmount"} type={n.includes("Date")?"date":["grossAmount","deductions","netAmount"].includes(n)?"number":"text"} name={n} value={form[n]} onChange={ch}/></div>
//         ))}
...
//         <div style={S.formGroup}><label style={S.label}>Status</label><select style={S.select} name="status" value={form.status} onChange={ch}>{["Pending","Paid","On Hold"].map(o=><option key={o}>{o}</option>)}</select></div>
//       </div>
//       <div style={S.btnRow}><button style={S.btnOutline} onClick={()=>setForm(empty)}>Clear</button><button style={S.btnYellow} onClick={save}>Save Payment</button></div>
...
//       <table style={S.table}>
//         <thead><tr>{["Vendor","Vehicle","Invoice","Trip","Gross","Deductions","Net","Date","Mode","UTR","Status"].map(h=><th style={S.th} key={h}>{h}</th>)}</tr></thead>
//         <tbody>{filtered.length?filtered.map((r,i)=>(
//           <tr key={i}><td style={S.td}>{r.vendorName}</td><td style={S.td}>{r.vehicleNumber}</td><td style={S.td}>{r.invoiceNumber}</td><td style={S.td}>{r.tripNumber}</td><td style={S.td}>₹{r.grossAmount}</td><td style={S.td}>₹{r.deductions}</td><td style={S.td}><b>₹{r.netAmount}</b></td><td style={S.td}>{r.paymentDate?new Date(r.paymentDate).toLocaleDateString("en-IN"):""}</td><td style={S.td}>{r.paymentMode}</td><td style={S.td}>{r.utrNumber}</td><td style={S.td}><span className={badge(r.status)}>{r.status}</span></td>
//         </tr>)):<tr><td colSpan={11} style={{...S.td,padding:28,color:"#aaa"}}>No records yet</td></tr>}</tbody>
//       </table>
//     </div>
//   );
// }
...
export default function Payment() {
  ...
  const load = () =>
    axios.get(`${BASE}/advance/vendor-payment`)
  ...
  const ch = (e) => {
    ...
    if (f.grossAmount && f.deductions) {
      f.netAmount = ((+f.grossAmount) - (+f.deductions)).toFixed(2);
    }
    setForm(f);
  };
  ...
  const save = async () => {
    try {
      await axios.post(`${BASE}/advance/vendor-payment`, form);
      setForm(empty);
      load();
  ...
  const getStatusBadge = (status) => {
    if (status === "Paid") return <span style={styles.badgeGreen}>Paid</span>;
    if (status === "On Hold") return <span style={styles.badgeRed}>On Hold</span>;
    return <span style={styles.badgeYellow}>Pending</span>;
  };

  return (
    <div style={styles.container}>
      <div style={styles.pageHeader}>VENDOR PAYMENTS</div>

      <div style={styles.innerPad}>

        {/* Form */}
        <div style={styles.sectionTitle}>Add Payment</div>
        <div style={styles.formGrid}>
          ...
          <div>
            <label style={styles.label}>Gross Amount (₹):</label>
            <input style={styles.input} type="number" name="grossAmount"
              value={form.grossAmount} onChange={ch} />
          </div>
          <div>
            <label style={styles.label}>Deductions (₹):</label>
            <input style={styles.input} type="number" name="deductions"
              value={form.deductions} onChange={ch} />
          </div>
          <div>
            <label style={styles.label}>Net Amount (₹) — Auto:</label>
            <input style={styles.readOnly} type="number" name="netAmount"
              value={form.netAmount} readOnly />
          </div>
          <div>
        ...
        <div style={styles.buttonRow}>
          ...
          <button style={styles.btnYellow} onClick={save}>Save Payment</button>
        </div>
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={styles.table}>
            <thead>
              <tr>
                {["Vendor", "Vehicle", "Invoice", "Trip", "Gross",
                  "Deductions", "Net", "Date", "Mode", "UTR", "Status"].map(h => (
            ...
            <tbody>
              {filtered.length > 0 ? filtered.map((r, i) => (
                <tr key={i} style={{ backgroundColor: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                  ...
                  <td style={styles.td}>₹{r.grossAmount}</td>
                  <td style={styles.td}>₹{r.deductions}</td>
                  <td style={styles.td}><b>₹{r.netAmount}</b></td>
                  <td style={styles.td}>
