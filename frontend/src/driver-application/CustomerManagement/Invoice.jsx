...
// const badge = s=>({Paid:"badge-green",Overdue:"badge-red",Unpaid:"badge-yellow",Partial:"badge-blue"}[s]||"badge-yellow");
// const empty = { invoiceNumber:"", customerName:"", vehicleNumber:"", tripFrom:"", tripTo:"", invoiceDate:new Date().toISOString().split("T")[0], dueDate:"", amount:"", gstAmount:"", totalAmount:"", status:"Unpaid" };
// export default function Invoice() {
//   const [form, setForm] = useState(empty);
//   const [records, setRecords] = useState([]);
//   const [search, setSearch] = useState("");
//   const load = ()=>axios.get(`${BASE}/customer/invoices`).then(r=>setRecords(r.data||[])).catch(()=>{});
//   useEffect(()=>{load();},[]);
//   const ch = e=>{const f={...form,[e.target.name]:e.target.value};if(f.amount&&f.gstAmount)f.totalAmount=((+f.amount)+(+f.gstAmount)).toFixed(2);setForm(f);};
//   const save = async()=>{try{await axios.post(`${BASE}/customer/invoices`,form);setForm(empty);load();}catch{alert("Save failed");}};
//   const filtered = records.filter(r=>(r.customerName||"").toLowerCase().includes(search.toLowerCase())||(r.invoiceNumber||"").toLowerCase().includes(search.toLowerCase()));
//   return (
//     <div style={S.container}>
//       <h2 className="sendo-heading">Invoice Management</h2>
//       <div style={S.formGrid}>
//         {[["Invoice Number","invoiceNumber"],["Customer Name","customerName"],["Vehicle Number","vehicleNumber"],["Trip From","tripFrom"],["Trip To","tripTo"],["Invoice Date","invoiceDate"],["Due Date","dueDate"],["Amount (₹)","amount"],["GST Amount (₹)","gstAmount"],["Total Amount (₹)","totalAmount"]].map(([l,n])=>(
//           <div style={S.formGroup} key={n}><label style={S.label}>{l}</label><input style={S.input} readOnly={n==="totalAmount"} type={n.includes("Date")?"date":["amount","gstAmount","totalAmount"].includes(n)?"number":"text"} name={n} value={form[n]} onChange={ch}/></div>
//         ))}
//         <div style={S.formGroup}><label style={S.label}>Status</label><select style={S.select} name="status" value={form.status} onChange={ch}>{["Unpaid","Paid","Overdue","Partial"].map(o=><option key={o}>{o}</option>)}</select></div>
//       </div>
//       <div style={S.btnRow}><button style={S.btnOutline} onClick={()=>setForm(empty)}>Clear</button><button style={S.btnYellow} onClick={save}>Save Invoice</button></div>
//       <div style={S.searchContainer}><input style={{...S.input,width:300}} placeholder="Search customer or invoice..." value={search} onChange={e=>setSearch(e.target.value)}/></div>
//       <table style={S.table}>
//         <thead><tr>{["Invoice No","Customer","Vehicle","From","To","Invoice Date","Due Date","Amount","GST","Total","Status"].map(h=><th style={S.th} key={h}>{h}</th>)}</tr></thead>
//         <tbody>{filtered.length?filtered.map((r,i)=>(
//           <tr key={i}><td style={S.td}>{r.invoiceNumber}</td><td style={S.td}>{r.customerName}</td><td style={S.td}>{r.vehicleNumber}</td><td style={S.td}>{r.tripFrom}</td><td style={S.td}>{r.tripTo}</td><td style={S.td}>{r.invoiceDate?new Date(r.invoiceDate).toLocaleDateString("en-IN"):""}</td><td style={S.td}>{r.dueDate?new Date(r.dueDate).toLocaleDateString("en-IN"):""}</td><td style={S.td}>₹{r.amount}</td><td style={S.td}>₹{r.gstAmount}</td><td style={S.td}><b>₹{r.totalAmount}</b></td><td style={S.td}><span className={badge(r.status)}>{r.status}</span></td>
//         </tr>)):<tr><td colSpan={11} style={{...S.td,padding:28,color:"#aaa"}}>No invoices yet</td></tr>}</tbody>
//       </table>
//     </div>
//   );
// }
...
export default function Invoice() {
  ...
  const load = () =>
    axios.get(`${BASE}/customer/invoices`)
  ...
  const ch = (e) => {
    ...
    if (f.amount && f.gstAmount) {
      f.totalAmount = ((+f.amount) + (+f.gstAmount)).toFixed(2);
    }
    setForm(f);
  };
  ...
  const save = async () => {
    try {
      await axios.post(`${BASE}/customer/invoices`, form);
      setForm(empty);
      load();
  ...
  return (
    <div style={styles.container}>
      <div style={styles.pageHeader}>INVOICE MANAGEMENT</div>
      ...
      <div style={styles.innerPad}>
        ...
        <div style={styles.formGrid}>
          ...
          <div>
            ...
            <input style={styles.input} type="date" name="dueDate"
              value={form.dueDate} onChange={ch} />
          </div>
          <div>
          ...
          <div>
            ...
            <input style={styles.input} type="number" name="gstAmount"
              value={form.gstAmount} onChange={ch} />
          </div>
          <div>
          ...
          <div>
            ...
            <select style={styles.input} name="status"
              ...
              {["Unpaid", "Paid", "Overdue", "Partial"].map(o => (
        ...
        <div style={styles.buttonRow}>
          ...
          <button style={styles.btnYellow} onClick={save}>Save Invoice</button>
        </div>
